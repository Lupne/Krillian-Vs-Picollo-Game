...
while run:
    ...
    for bullet in bullets:
        if bullet.y-5 < goblin.hitbox[1]+goblin.hitbox[3] and bullet.y + 5> goblin.hitbox[1]:
            if bullet.x+5>goblin.hitbox[0] and bullet.x-5<goblin.hitbox[0]+goblin.hitbox[2]:
                goblin.hit()
                score+=1
    ...
    if keys[pygame.K_SPACE] and shootloop==0:
        if man.left:
            ...
            facing=1
        if len(bullets)<30:
            bullets.append(porjective(man.x+man.width//2,man.y+man.height//2,(0,0,0),facing))
        shootloop=1
